...
public class ClientCourier {
    ...
    // метод проверяет успешность создания курьера
    ...
    // метод отправляет POST-запрос для создания нового курьера
    ...
    // метод отправляет POST-запрос для авторизации курьера
    @Step("Send POST request to api/v1/courier/login")
    public ValidatableResponse loginCourier(CredentialsCourier creds) {
        return given().log().all()
                .contentType(ContentType.JSON)
                .baseUri(BASE_URL)
                .body(creds)
    ...
    // метод проверяет успешную авторизацию курьера
    ...
    // метод отправляет DELETE-запрос для удаления курьера по его id
    ...
    // метод проверяет успешность удаления курьера
    ...
    // метод проверяет ошибку при попытке создать курьера с уже существующим логином
    ...
    // метод проверяет ошибку при создании курьера с неполными данными
    ...
    // метод проверяет ошибку при попытке авторизоваться с неполными данными
    ...
    // метод проверяет ошибку при попытке авторизоваться с неверными учетными данными
    @Step("Compare error code & text with invalid creds response")
